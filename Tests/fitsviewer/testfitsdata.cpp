...
{
    runParallelSolvers(SolverUtils::MULTI_HEDGE);
}
...
void TestFitsData::testHedgeSolver()
{
    const QString probe = QFINDTESTDATA("ngc4535-autofocus1.fits");
    if (probe.isEmpty())
        QSKIP("Missing fixture files");
    const QString dir = QFileInfo(probe).absolutePath();

    if (!ensureIndexFiles())
        QSKIP("No astrometry index files found -- skipping hedge solver test");

    SolverUtils::setMultiAlgorithmOverride(SolverUtils::MULTI_HEDGE);
    ...
    for (const auto &fname : files)
    {
        ...
        QVERIFY(image->loadFromFile(QString("%1/%2").arg(dir, fname)).result());

        auto profiles = Ekos::getDefaultAlignOptionsProfiles();
        auto parameters = profiles.at(Options::solveOptionsProfile());
        Options::setSolverType(0);

        QSharedPointer<SolverUtils> solver(new SolverUtils(parameters, 30), &QObject::deleteLater);
        solver->useScale(false, 0, 0);
        solver->usePosition(false, 0, 0);
        ...
        QVERIFY2(success, qPrintable(QString("%1 failed to solve").arg(fname)));
        QVERIFY(solution.pixscale > 0);
        qInfo() << QString("%1: RA %2 DEC %3 scale %4 in %5s")
                       .arg(fname)
                       .arg(solution.ra, 0, 'f', 3)
                       .arg(solution.dec, 0, 'f', 3)
