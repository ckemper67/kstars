...
void TestDonutsRegistrar::testMathRecovery()
{
    ...
    // Test 3: Combined translation + rotation
    {
        ...
        // Phase correlation measures a single bulk shift per quadrant.  Under combined
        // rotation+translation, stars within a quadrant span different y-positions so each
        // shifts by a slightly different x-amount (e.g. 1.99-2.34 px for Q0 at 0.2 deg).
        // The correlation peak lands near the middle of this spread rather than at the
        // flux-weighted centroid, giving a ~0.12 deg dtheta error.  Translation accuracy
        // is still sub-pixel; the rotation tolerance reflects this 1-D projection limit.
        QVERIFY(std::abs(res.dx - 2.0) < 0.2);
        QVERIFY(std::abs(res.dy - 2.0) < 0.2);
        QVERIFY(std::abs(gotDeg - 0.2) < 0.15);
    }
}

void TestDonutsRegistrar::testOffCenterRotation()
{
...
TestDonutsRegistrar::Buffer TestDonutsRegistrar::makeStarField(int w, int h)
{
...
void TestDonutsRegistrar::testStarDensity()
{
    ...
    for (int n : counts)
    {
        ...
        auto cur = TestDonutsRegistrar::transformBuffer(
            buf, W, H, DX, DY, 0.0);
        Donuts::Registrar g;
